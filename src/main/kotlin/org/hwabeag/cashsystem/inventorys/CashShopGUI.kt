...
class CashShopGUI(private val player: Player) {
    ...
    private val inv: Inventory = Bukkit.createInventory(
        null,
        line * 9 + 9,
        MessageManager.message("gui.title.shop", mapOf("shop" to shopName))
    )
    ...
    private fun addDesign() {
        ...
        val balance = CashSystem.cashRepository.getCash(player.name)
        ...
        inv.setItem(
            ...
            GuiDesign.createStatusItem(
                nameKey = "gui.footer.shop_status_name",
                loreKey = "gui.footer.shop_status_lore",
                placeholders = mapOf("page" to page, "cash" to balance)
            )
        )
    }
}
