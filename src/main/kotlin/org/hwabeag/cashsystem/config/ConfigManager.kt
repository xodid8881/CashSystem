...
object ConfigManager {
    ...
    fun initialize(cashSystem: CashSystem) {
        ...
        configSet["cash-system"] = ConfigMaker(plugin, path, "CashSystem.yml")
        configSet["setting"] = ConfigMaker(plugin, path, "config.yml")
        configSet["player"] = ConfigMaker(plugin, path, "Player.yml")
        configSet["message-kor"] = ConfigMaker(plugin, path, "message_kor.yml")
        configSet["message-eng"] = ConfigMaker(plugin, path, "message_eng.yml")
    ...
    fun getConfig(fileName: String): FileConfiguration {
        return requireNotNull(configSet[fileName]) { "설정 파일을 찾을 수 없습니다: $fileName" }.getConfig()
    }
    ...
    private fun loadDefaults() {
        ...
        settingConfig.addDefault("language", "kor")
